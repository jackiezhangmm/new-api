...
export function CanvasNodeHoverToolbar({
    ...
    const canRetry = canvasCapabilities.generation && node.metadata?.status === "error" && !(isVideo && Boolean(node.metadata?.videoTaskId) && !hasVideo);
    ...
    const nodeToolbarTools: ToolbarTool[] = [
        ...
        ...(isText && canvasCapabilities.generation ? [{ id: "generateImage", title: t("canvas.node.generateImage"), label: t("canvas.node.generate"), icon: <ImageIcon className="size-4" />, onClick: () => onGenerateImage(node) }] : []),
        ...(isConfig && canvasCapabilities.generation ? [{ id: "config", title: t("canvas.configNode.title"), label: t("canvas.configNode.title"), icon: <Settings2 className="size-4" />, onClick: () => onToggleDialog(node) }] : []),
    ...
    return (
        <Tooltip title={title} placement="top" mouseEnterDelay={0.2} color="#ffffff" styles={{ root: { color: "#242529", boxShadow: "0 8px 24px rgba(15,23,42,.16)", fontSize: 13, fontWeight: 500 } }}>
            <button
                type="button"
                disabled={disabled}
                className={`group relative flex h-12 items-center whitespace-nowrap px-1.5 ${danger ? "text-[#ef4444]" : ""} ${disabled ? "opacity-40 cursor-not-allowed" : ""}`}
                onClick={disabled ? undefined : onClick}
                aria-label={title}
            >
                <span className={`flex h-9 items-center ${hasText ? "gap-2 px-2.5" : "justify-center px-2"} rounded-lg transition ${disabled ? "" : "group-hover:bg-[#f0f0f1]"} ${active ? "bg-[#eeeeef]" : ""}`}>
                    {icon}
